...
<div class="sc-page-body">
    ...
    <div class="sc-grid">
        ...
        ?>
        <div class="vc" onclick="window.location='<?= $lien ?>'">

            <div class="vc-img-wrap">
                <img src="<?= $imgSrc ?>" alt="<?= htmlspecialchars($p['NOM_PRODUIT']) ?>" loading="lazy">

                <?php if ($isPromo): ?>
                <span class="vc-badge vc-badge-promo">Promo</span>
                <?php else: ?>
                <span class="vc-badge vc-badge-new">New</span>
                <?php endif; ?>

                <button class="vc-wish"
                        onclick="event.stopPropagation(); vcWish(<?= (int)$p['ID_PRODUIT'] ?>, this)"
                        title="Ajouter aux favoris">
                    <i class="far fa-heart"></i>
                </button>
                <button class="vc-cart"
                        onclick="event.stopPropagation(); vcCart(<?= (int)$p['ID_PRODUIT'] ?>, this)"
                        title="Ajouter au panier">
                    <i class="fas fa-shopping-bag"></i>
                </button>

                <button class="vc-add-overlay"
                        onclick="event.stopPropagation(); window.location='<?= $lien ?>'">
                    <i class="fas fa-shopping-bag" style="margin-right:8px;"></i>Voir le produit
                </button>
            </div>

            <div class="vc-info">
                <div class="vc-cat"><?= htmlspecialchars($scNom) ?></div>
                <div class="vc-name"><?= htmlspecialchars($p['NOM_PRODUIT']) ?></div>
                <div class="vc-price">
                    <?php if ($isPromo): ?>
                        <span class="pp-promo"><?= number_format((float)$p['PRIX_PROMO'], 0) ?> DH</span>
                        <span class="pp-old"><?= number_format((float)$p['PRIX'], 0) ?> DH</span>
                    <?php else: ?>
                        <span class="pp-normal"><?= number_format((float)$p['PRIX'], 0) ?> DH</span>
                    <?php endif; ?>
                </div>
            </div>
