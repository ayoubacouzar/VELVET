...
// ── Recherche — Lancer (redirige vers produits.php) ──
function doSearch() {
    const q = document.getElementById('searchInput').value.trim();
    if (!q) return;
    // Détecter si on est à la racine ou dans un sous-dossier
    const isRoot = !window.location.pathname.includes('/client/');
    const base   = isRoot ? 'client/' : '';
    window.location.href = base + 'produits.php?q=' + encodeURIComponent(q);
}
...
// ── Quick add to cart (from homepage — silent, updates badge) ──
