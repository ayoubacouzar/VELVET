    ...
    <div class="main-content main-content-profile">
        <div class="profile-wrap">
            <div class="profile-card">
                ...
                <div id="phpToast" data-msg="<?= htmlspecialchars(reset($errors)) ?>" data-type="error"></div>
                <?php endif; ?>
                ...
                <form method="POST" id="profileForm" novalidate>
                    <div class="field-row">
                        <div class="field-group">
                            ...
                            <div class="input-wrap">
                                <input type="text" name="prenom" value="<?= htmlspecialchars($_POST['prenom'] ?? $admin['PRENOM_ADMIN'] ?? '') ?>">
                            </div>
                        </div>
                        <div class="field-group">
                            <label>Nom</label>
                            <div class="input-wrap">
                                <input type="text" name="nom" value="<?= htmlspecialchars($_POST['nom'] ?? $admin['NOM_ADMIN'] ?? '') ?>">
                            </div>
                        </div>
                    </div>
                    <div class="field-group">
                        <label>E-mail</label>
                        <div class="input-wrap">
                            <input type="email" name="email" maxlength="254" value="<?= htmlspecialchars($_POST['email'] ?? $admin['EMAIL_ADMIN'] ?? '') ?>">
                        </div>
                    </div>
                    ...
                    <div class="field-group">
                        ...
                        <div class="input-wrap">
                            <input type="password" name="password" id="pwd" maxlength="30" class="has-eye" oninput="checkPassword(this.value)">
                    ...
                    <div class="field-group">
                        ...
                        <div class="input-wrap">
                            <input type="password" name="password_confirm" id="pwd2" maxlength="30" class="has-eye">
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('profileForm').addEventListener('submit', function(e) {
        ...
        if (!prenom || prenom.length < 2) {
            e.preventDefault();
            showToast(prenom ? 'Le prénom doit contenir au moins 2 caractères.' : 'Le prénom est obligatoire.', 'error');
            return;
        }
        if (!nom || nom.length < 2) {
            e.preventDefault();
            showToast(nom ? 'Le nom doit contenir au moins 2 caractères.' : 'Le nom est obligatoire.', 'error');
            return;
        }
        if (!email) {
            e.preventDefault();
            showToast("L'adresse e-mail est obligatoire.", 'error');
            return;
        }
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
            e.preventDefault();
            showToast("L'adresse e-mail n'est pas valide.", 'error');
            return;
        }
        if (pwd) {
            if (pwd.length < 8 || pwd.length > 30) {
                e.preventDefault();
                showToast('Le mot de passe doit contenir entre 8 et 30 caractères.', 'error');
                return;
            }
            if (!/[0-9]/.test(pwd)) {
                e.preventDefault();
            ...
            if (/<|>|script|eval|alert|SELECT|INSERT|UPDATE|DELETE|DROP|--|;/i.test(pwd)) {
                e.preventDefault();
                showToast('Le mot de passe contient des caractères non autorisés.', 'error');
                return;
            }
            if (pwd !== pwd2) {
                e.preventDefault();
